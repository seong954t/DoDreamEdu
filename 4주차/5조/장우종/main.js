...
function checkDuplicatedNumInList(lotto_nums){
   for(var i=0; i<lotto_nums.length; i++){
       for(var j=i+1; j<lotto_nums.length; j++){
...
function getMachedNum(lotto_nums){
    var cnt = 0;
    for(var i=0; i<lotto_nums.length; i++){
        for(var j=0; j<winning_numbers.length; j++){
            if(lotto_nums[i] == winning_numbers[j])
                 cnt++;
        }
    }
    return cnt;
...
function checkWinLotto(){
  ...
  for(let i in user_lotto_numbers_list) {
      ...
      else if(result[i].textContent == "1등"){
          alert("1등이에요! 축하합니다! : " + user_lotto_numbers_list[i]);
      }
  }
