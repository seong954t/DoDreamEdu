...
function getLottoNums(){
    ...
    while(lotto_nums.length<6){
        ...
        if(!checkDuplicatedNum(lotto_nums,randomNum)){
            lotto_nums.push(randomNum)
        }
    }
    return lotto_nums;
}
...
function getWinningResult(lotto_nums){
   switch(getMachedNum(lotto_nums)){
       case 6:
            return '1등';
            break;
       case 5:
            return checkDuplicatedNum(lotto_nums,bonus_number)? '2등':'3등';
            break;
       case 4:
            return '4등'
            break;
